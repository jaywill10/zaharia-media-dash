...
app.get('/api/news', async (_req, res) => {
  try {
    ...
    const rssResp = await fetch(url);
    if (!rssResp.ok) throw new Error('msn fetch');
    ...
    try {
      const rss = await fetch('https://feeds.bbci.co.uk/news/rss.xml').then(r=>r.text());
      const data = await parseStringPromise(rss, { explicitArray:false });
      const rawItems = data?.rss?.channel?.item || [];
      const items = (Array.isArray(rawItems) ? rawItems : [rawItems]).slice(0,5).map(it => ({
        title: it.title || '',
      ...
      res.json({ items: [], enabled:true, error:'Failed to fetch news' });
    }
  }
});
